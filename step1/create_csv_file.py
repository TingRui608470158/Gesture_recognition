...
def Write_midpoint_csv(single_midpoint):
    with open('../../annotation/step1_output.csv', 'a', newline='') as csvfile:    # 開啟輸出的 CSV 檔案
        writer = csv.writer(csvfile)        # 建立 CSV 檔寫入器
        writer.writerow(single_midpoint)    # 寫入一列資料
    ...
    for path, im, im0s, vid_cap, s in dataset:
        for class_index, class_name in enumerate(gesture_class):
            if gesture_class[class_index] in path:
                gesture_id = class_index
                print("stop")
                print("gesture_id = ",gesture_id)
                print("class_name = ",gesture_class[class_index])
##############################################################################################################################################
        with dt[0]:
...
############################################################################################################################################
                ...
                my_output = det.cpu().detach().numpy()          #輸出值
                csv_x =  (my_output[0][0] + my_output[0][2])/2  #中心點x
                csv_y =  (my_output[0][1] + my_output[0][3])/2  #中心點y
                orign_point.append(csv_x)   #未標準化原始點x
                orign_point.append(csv_y)   #未標準化原始點y

                if csv_count == 0:              #當沒有起始點的時候
                    start_point = csv_x,csv_y   #賦予起始點
                csv_count = csv_count+1

                csv_x = csv_x  - start_point[0] #相對位置x
                csv_y = csv_y  - start_point[1] #相對位置y
                
                if First_point == True:
                    csv_frame_points.append(gesture_id)
                    First_point = False
